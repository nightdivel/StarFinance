...
## 2a. История и настройки инструментов (tools_service)

Сервис `tools_service` (порт 3010) теперь отвечает за:
- `/api/tools/runs` — история запусков инструментов
- `/api/tools/settings` — настройки инструментов
Эти ручки проксируются через основной backend и требуют JWT. Все остальные ручки инструментов остались в монолите.
...
### 7.4 Бизнес-правила
- Фоновое изображение ограничено размером (лимиты задаются на уровне HTTP-сервера и Express).
...
- **HTTPS**: Терминируется Caddy (Let's Encrypt), сертификаты хранятся в Docker volume `caddy_data`.
- **Ограничения доступа**:
...
- **Развертывание**: Docker Compose с контейнерами:
  - `economy`: Node.js-приложение (порт 3000 внутри docker-сети).
  ...
  - `caddy`: Reverse proxy/TLS (порты 80/443).
- **Публикация**: приложение доступно по адресу `https://<domain>/economy/` (Caddy проксирует под поддиректорию `/economy` на `economy:3000`).
- **Переменные окружения**:
  - `backend/.env`: Секреты (`JWT_SECRET`, Discord OAuth), порты, URL.
  - `frontend/.env`: Параметры фронтенда (без секретов).
  - В продакшене `FRONTEND_URL=https://<domain>/economy`.
